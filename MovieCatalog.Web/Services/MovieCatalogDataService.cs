...
public class MovieCatalogDataService : IMovieCatalogDataService
{
    ...
    {
        var titleGenres = await ctx.TitleGenres
            .Include(tg => tg.Genre) 
            .ToListAsync();

        var genreCounts = titleGenres
            .GroupBy(tg => tg.Genre)
            .Select(g => new
            {
                Genre = g.Key,
        ...
        return genreCounts.ToDictionary(g => g.Genre, g => g.Count);
    }
    ...
    public Task<List<Title>> GetTitlesAsync(int v1, int v2, TitleFilter filter, TitleSort sort)
    {
